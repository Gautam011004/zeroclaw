    ...
    pub async fn turn_streamed_with_steering_state(
        ...
        for round in 0..self.config.resolved.max_tool_iterations {
            ...
            let round_loop = crate::agent::loop_::TOOL_LOOP_COST_TRACKING_CONTEXT.scope(
                Some(cost_context.clone()),
                // Per-turn attachment scope: tools can publish media (QR codes,
                // images, PDFs, etc.) without threading binary payloads through
                // ToolResult or every intermediate runtime API. The orchestrator
                // drains this task-local context when constructing the outbound
                // SendMessage for the current turn.
                zeroclaw_api::attachments::TURN_ATTACHMENT_CONTEXT.scope(
                    Some(zeroclaw_api::attachments::TurnAttachmentContext::default()),
                    crate::agent::tool_receipts::scope_receipts(
                        receipt_scope.clone(),
                        crate::agent::loop_::run_tool_call_loop(crate::agent::loop_::ToolLoop {
                            exec: crate::agent::loop_::ResolvedAgentExecution::resolve(
